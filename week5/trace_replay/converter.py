...
def fetch_trace(trace_id: str, client: object) -> TraceData:
    ...
    run_row = client.query_json_rows(
        ...
        """
        # removed Limit 1 to allow combining multiple 'agent run' spans if they exist
    )
    rows = run_row['rows']
    if not rows:
    ...
    """
    # combine ALL messages (optional safety merge)
    all_messages = None
    total_input = 0
    total_output = 0
    ...
    for r in rows:
        ...
        # keep most complete message set
        if all_messages is None and msg:
            all_messages = msg
